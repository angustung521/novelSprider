...
def indexPage():
    with  sync_playwright() as p:
        ...
        # 导航到目标网址
        try:
            url = "https://www.bqgoo.cc/shu/123426/"
            page.goto(url,timeout=9000)
            print(page.content())
            page.wait_for_selector('.listmain')
            dd_elements = page.query_selector_all('.listmain >dl > dd')
    ...
    with sync_playwright() as p:
        for singlePage in allDetailPages:
            ...
            try:
                ...
                singlePageContent = re.sub(r'&[\da-fA-F]{1,4};', '', content_text)
