...
def indexPage():
    with  sync_playwright() as p:
        ...
        try:
            ...
            dd_elements = page.query_selector_all('.listmain >> dd')

         # 遍历每个li元素
            for dd in dd_elements:
                # 在每个li中获取所有的a元素
                a_elements = dd.query_selector_all('a')
    ...
    with sync_playwright() as p:
        for singlePage in allDetailPages:
            ...
            try:
                ...
                title_element = page.wait_for_selector('h1.wap_none')
...
# 格式化文本
# import re
# with open("可爱过敏原.txt", 'r', encoding='utf-8') as file:
#     allContent = file.read()
#
# pattern_to_remove = r'无尽的昏迷过后.*御兽师\？'
# singlePageContent = re.sub(pattern_to_remove, '', allContent)
# with open("可爱过敏原.txt", 'w', encoding='utf-8') as file:
#     file.write(singlePageContent)
#     file.close()